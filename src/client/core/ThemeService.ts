/**
 * ThemeService - Manages global CSS variables for the agent platform.
 *
 * Supports:
 * - Legacy IDE themes (vs-dark, hc-black, etc.)
 * - Agent variants (command, alert, observer) via data-theme attribute
 */
...
export type AgentVariant = 'command' | 'alert' | 'observer';

export class ThemeService {
    private ide: IDE;
    private currentVariant: AgentVariant = 'command';
    private alertTimer: ReturnType<typeof setTimeout> | null = null;

    constructor(ide: IDE) {
        this.ide = ide;

        // Apply on startup (async attempt since settings might be an extension)
        this.initializeTheme();

        // Apply default agent variant
        document.documentElement.setAttribute('data-theme', this.currentVariant);

        // Listen for changes
        this.ide.commands.on('configuration.changed', (e: unknown) => {
    ...
    private initializeTheme() {
        // Try to get theme from settings service if it exists
        const settings = this.ide.services.getService<any>('settings');
        const currentTheme = settings ? settings.get('editor.theme') : 'ide-dark';
        this.applyTheme(currentTheme || 'ide-dark');
    }
    ...
    /**
     * Get the current agent variant.
     */
    public getAgentVariant(): AgentVariant {
        return this.currentVariant;
    }

    /**
     * Switch to an agent theme variant (command, alert, observer).
     * These are defined in agent-theme.css via [data-theme="..."] selectors.
     */
    public setAgentVariant(variant: AgentVariant): void {
    ...
    public triggerAlert(durationMs: number = 5000): void {
        ...
        this.setAgentVariant('alert');

        this.alertTimer = setTimeout(() => {
            if (this.currentVariant === 'alert') {
                this.setAgentVariant(previousVariant);
            }
