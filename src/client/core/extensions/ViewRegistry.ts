...
import { IDE } from '../IDE';
import { TabEvents } from '../TabService.js';
...
export class ViewRegistry {
    ...
    private setupTabListeners() {
        ...
        this.ide.commands.on(TabEvents.TAB_OPENED, async (data: any) => {
            const { tabId, config, contentPanel } = data;
    ...
    public registerProvider(location: ViewLocation, provider: ViewProvider): void {
        ...
        if (locationMap.has(provider.id)) {
            console.warn(`ViewRegistry: Provider "${provider.id}" is already registered in "${location}". Overwriting.`);
            // Clean up the old one if necessary
            this.unregisterProvider(location, provider.id);
        }

        locationMap.set(provider.id, provider);
        this.ide.layout.statusBar.setMessage(`Registered View: ${provider.name} in ${location}`);
    }

    /**
     * Get all registered providers grouped by location for introspection views.
     */
    public getRegisteredProviders(): Map<string, Array<{ id: string; name: string }>> {
        const result = new Map<string, Array<{ id: string; name: string }>>();
        for (const [location, providerMap] of this.providers) {
            const views: Array<{ id: string; name: string }> = [];
            for (const [id, provider] of providerMap) {
                views.push({ id, name: provider.name });
            }
            result.set(location, views);
        }
        return result;
    }
    ...
    public unregisterProvider(location: ViewLocation, providerId: string): void {
        ...
        if (provider) {
            ...
            this.ide.layout.statusBar.setMessage(`Unregistered View: ${provider.name}`);
        }
    }
    ...
    public async renderView(location: ViewLocation, providerId: string): Promise<void> {
        ...
        if (location === 'left-panel' || location === 'right-panel') {
            const sidebarContent = targetPanel.querySelector('.sidebar-content');
            if (sidebarContent instanceof HTMLElement) {
                targetPanel = sidebarContent;
            }
        } else if (location === 'bottom-panel') {
            const bottomContent = targetPanel.querySelector('.bottom-panel-content');
            if (bottomContent instanceof HTMLElement) {
        ...
        if (location !== 'center-panel') {
            ...
            // Center panel: open as a custom tab via TabService
            const provider = this.getProvider('center-panel', providerId);
            if (!provider) return;

            this.ide.tabs.openTab({
                id: providerId,
                title: provider.name,
