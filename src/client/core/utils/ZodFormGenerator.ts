...
const CONTEXT_KEYS = new Set(['threadId', 'side', 'batchId']);
    ...
    for (const [key, fieldSchema] of Object.entries(shape)) {
        ...
        // ── Boolean ──────────────────────────────────────────────────────
            ...
            fields.push({
                ...
                getValue: () => {
                    const cb = toggle.getElement().querySelector('input[type="checkbox"]');
        ...
        // ── Enum ─────────────────────────────────────────────────────────
            ...
            fields.push({
                ...
                getValue: () => {
                    const sel = select.getElement().querySelector('select');
                    return (sel instanceof HTMLSelectElement) ? sel.value : opts[0];
                },
                isAutoPopulated: false,
            });
        }
        // ── Nested Object (Vector3, etc.) ────────────────────────────────
            ...
            if (shapeKeys.includes('x') && shapeKeys.includes('y') && shapeKeys.includes('z') && shapeKeys.length <= 4) {
                ...
                fields.push({
                    ...
                    getValue: () => {
                        const area = textarea.getElement().querySelector('textarea');
                        const val = (area instanceof HTMLTextAreaElement) ? area.value : '';
