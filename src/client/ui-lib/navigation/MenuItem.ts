...
export class MenuItem extends BaseComponent<MenuItemProps> {
    ...
    constructor(props: MenuItemProps) {
        ...
        // Listen for close signal from sub-menus (bubbles up)
        this.element.addEventListener('menu-close', (e: Event) => {
            const customEvent = e as CustomEvent;
            if (customEvent.detail && customEvent.detail.item === this) return;
    ...
    private onDocumentClick(e: MouseEvent): void {
        if (this.isOpen && e.target instanceof Node && !this.element.contains(e.target)) {
            this.close();
        }
    }
