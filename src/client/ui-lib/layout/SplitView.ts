...
export class SplitView extends BaseComponent<SplitViewProps> {
    ...
    private currentSizes: string[] = []; // Store flex values
    ...
    public render(): void {
        ...
        // If number of panes changed, reset currentSizes
        if (this.currentSizes.length !== panes.length) {
            this.currentSizes = panes.map((_, index) => 
                index < initialSizes.length ? `0 0 ${initialSizes[index]}%` : '1'
            );
        }
        ...
        panes.forEach((pane, index) => {
            ...
            Object.assign(paneWrapper.style, {
                flex: this.isStacked ? '0 0 auto' : this.currentSizes[index],
                overflow: 'hidden',
                position: 'relative',
    ...
    private setupDrag(sash: HTMLElement, leftPaneIndex: number): void {
        ...
        const onMouseMove = (e: MouseEvent) => {
            ...
            const minLeft = this.props.minSizes?.[leftPaneIndex] ?? 50;
            const maxLeft = this.props.maxSizes?.[leftPaneIndex] ?? Infinity;
            const minRight = this.props.minSizes?.[leftPaneIndex + 1] ?? 50;
            const maxRight = this.props.maxSizes?.[leftPaneIndex + 1] ?? Infinity;

            if (leftSize >= minLeft && leftSize <= maxLeft && rightSize >= minRight && rightSize <= maxRight) {
                const leftFlex = `0 0 ${leftSize}px`;
                const rightFlex = `1 1 auto`;
                leftPane.style.flex = leftFlex;
                rightPane.style.flex = rightFlex;
                
                // Update currentSizes for persistence
                this.currentSizes[leftPaneIndex] = leftFlex;
                this.currentSizes[leftPaneIndex + 1] = rightFlex;
            }
        };
