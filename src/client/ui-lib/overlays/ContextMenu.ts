...
export class ContextMenu extends BaseComponent<{ items: ContextMenuItem[], x: number, y: number }> {
    ...
    public static show(itemsOrOptions: ContextMenuItem[] | { items: ContextMenuItem[], x: number, y: number }, x?: number, y?: number): ContextMenu {
        if (Array.isArray(itemsOrOptions)) {
            return new ContextMenu(itemsOrOptions, x || 0, y || 0);
        }
        return new ContextMenu(itemsOrOptions.items, itemsOrOptions.x, itemsOrOptions.y);
    }
    ...
    private renderItem(item: ContextMenuItem): HTMLElement {
        ...
        if (item.disabled) {
            ...
            if (!isContextMenuSubmenu(item)) {
                el.onclick = (e) => {
                    e.stopPropagation();
                    const action = (item as ContextMenuAction).action || (item as ContextMenuAction).onClick;
                    if (action) action();
                    this.disposeAll();
                };
            }
