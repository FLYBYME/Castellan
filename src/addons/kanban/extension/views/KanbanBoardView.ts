...
export class KanbanBoardView implements ViewProvider {
    ...
    private renderTaskCard(task: KanbanTask): HTMLElement {
        ...
        if (this.selectedTaskId === (task as any).id) {
        ...
        card.getElement().addEventListener('dragstart', (e: DragEvent) => {
            e.dataTransfer?.setData('text/plain', (task as any).id);
        ...
        priorityRow.appendChildren(dot, prioText);
