...
export interface ConfigurationProperty<T = unknown> {
...
export class ConfigurationRegistry {
     ...
     */
    public validate(key: string, value: unknown): string | null {
        ...
        switch (prop.type) {
            ...
            case 'number':
                ...
                if (prop.min !== undefined && (value as number) < prop.min) {
                    return `"${key}" must be at least ${prop.min}.`;
                }
                if (prop.max !== undefined && (value as number) > prop.max) {
